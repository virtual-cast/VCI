...
namespace UniGLTF.Legacy
{
    ...
    /// GLTF importer
    /// </summary>
    public class LegacyImporterContext : IDisposable
    {
        #region MeasureTime
        bool m_showSpeedLog
#if VRM_DEVELOP
        ...
        public LegacyImporterContext(IShaderStore shaderStore)
        {
            m_shaderStore = shaderStore;
        }

        public LegacyImporterContext(IMaterialImporter materialImporter)
        {
            m_materialImporter = materialImporter;
        }

        public LegacyImporterContext()
        {
        }
        ...
        public void CreateTextureItems(UnityPath imageBaseDir = default(UnityPath))
        {
            ...
            for (int i = 0; i < GLTF.textures.Count; ++i)
            {
                ...
                var image = GLTF.images[gltfTexture.source];
                    ...
                    for (int i = 0; i < GLTF.meshes.Count; ++i)
                    {
                        ...
                        parent.AddTask(Scheduler.ThreadPool,
                                () =>
                                {
                                    ...
                                    {
                                        return meshImporter.ReadMesh(GLTF, index, Axises.Z.Create());
                                    }
                                })
                ...
                .ContinueWith(Scheduler.MainThread, _ =>
                {
                    using (MeasureTime("AnimationImporter"))
                    {
                        if (GLTF.animations != null && GLTF.animations.Any())
                        {
                            ...
                            AnimationClips = AnimationImporter.Import(GLTF, Root, Nodes, AnimationClips, Axises.Z).ToList();
        ...
        IEnumerator BuildMesh(MeshImporter.MeshContext x, int i)
        {
            ...
            {
                var meshWithMaterials = MeshImporter.BuildMeshAsync(default(ImmediateCaller), materialIndex => m_materials[materialIndex], x).Result;
        ...
        IEnumerator LoadMeshes()
        {
            ...
            for (int i = 0; i < GLTF.meshes.Count; ++i)
            {
                var meshContext = meshImporter.ReadMesh(GLTF, i, Axises.Z.Create());
                var meshWithMaterials = MeshImporter.BuildMeshAsync(default(ImmediateCaller), materialIndex => m_materials[materialIndex], meshContext).Result;
                var mesh = meshWithMaterials.Mesh;
                if (string.IsNullOrEmpty(mesh.name))
                {
                    mesh.name = string.Format("UniGLTF import#{0}", i);
        ...
        public void ExtractImages(UnityPath prefabPath)
        {
            ...
            for (int i = 0; i < GLTF.images.Count; ++i)
            {
                ...
                {
                    string textureName = GLTF.images[i].name;
                    var byteSegment = GLTF.GetImageBytes(Storage, i);
