...
namespace VCIJSON
{
    public static class JsonSchemaValidatorFactory
    {
            ...
            ItemJsonSchemaAttribute ia = null)
        {
            switch (valueType)
            {
                case ValueNodeType.Integer:
                {
                    ...
                    if (a != null)
                    {
                        if (!double.IsNaN(a.Minimum)) v.Minimum = (int) a.Minimum;
                        if (a.ExclusiveMinimum) v.ExclusiveMinimum = a.ExclusiveMinimum;
                        if (!double.IsNaN(a.Maximum)) v.Maximum = (int) a.Maximum;
                        if (a.ExclusiveMaximum) v.ExclusiveMaximum = a.ExclusiveMaximum;
                        if (a.MultipleOf != 0) v.MultipleOf = (int) a.MultipleOf;
                    }
                ...
                case ValueNodeType.Array:
                {
                    ...
                    if (a != null)
                    {
                        if (a.MinItems != 0) v.MinItems = a.MinItems;
                        if (a.MaxItems != 0) v.MaxItems = a.MaxItems;

                        if (t != null)
                        {
                            if (ia == null) ia = new ItemJsonSchemaAttribute();

                            Type elementType = null;
                            if (t.IsArray)
                                elementType = t.GetElementType();
                            else if (t.GetIsGenericList()) elementType = t.GetGenericArguments().First();
                ...
                case ValueNodeType.Object:
                {
                    ...
                    else
                    {
                        ...
                        if (a != null)
                        {
                            if (a.MinProperties > 0) v.MinProperties = a.MinProperties;

                            // props
                            foreach (var prop in GetProperties(t, a.ExportFlags))
                            {
                                v.Properties.Add(prop.Key, prop.Schema);
                                if (prop.Required) v.Required.Add(prop.Key);
                                if (prop.Dependencies != null) v.Dependencies.Add(prop.Key, prop.Dependencies);
                            }
                        }
        ...
        private static ValueNodeType ToJsonType(Type t)
        {
            ...
            if (s_typeMap.TryGetValue(t, out jsonValueType)) return jsonValueType;

            if (t.IsArray) return ValueNodeType.Array;
            if (t.GetIsGenericList()) return ValueNodeType.Array;
