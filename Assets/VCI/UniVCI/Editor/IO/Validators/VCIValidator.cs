...
namespace VCI
{
    public static class VCIValidator
    {
            ...
            foreach (var renderer in renderers)
            {
                switch (renderer)
                {
                    ...
                    case SkinnedMeshRenderer _:
                        break;
                    default:
                        var errorText = VCIConfig.GetFormattedText($"error{(int)ValidationErrorType.InvalidComponent}", renderer.GetType().Name);
                        throw new VCIValidatorException(ValidationErrorType.RootIsAnimated, renderer, errorText);
                }
            }
        }
        ...
        private static void CheckSelfGameObjAnimated(GameObject gameObject)
        {
            ...
            foreach (var animationClip in animationClips)
            {
                foreach (var animationCurveBindings in AnimationUtility.GetCurveBindings(animationClip))
                {
                    ...
                    throw new VCIValidatorException(ValidationErrorType.RootIsAnimated, animationClip, errorText);
                }
            }
#endif
        }
        ...
        private static void CheckRotationAnimationInterpolation(GameObject gameObject)
        {
            ...
            foreach (var animationClip in animationClips)
            {
                foreach (var animationCurveBindings in AnimationUtility.GetCurveBindings(animationClip))
                {
                    ...
                    var errorText = VCIConfig.GetText($"error{(int)ValidationErrorType.RotationInterpolationIsEulerAngles}");
                    throw new VCIValidatorException(ValidationErrorType.RotationInterpolationIsEulerAngles, animationClip, errorText);
                }
            }
#endif
        }
        ...
        private static void ValidateSpringBones(GameObject gameObject)
        {
            ...
            foreach (var springBone in springBones)
            {
                ...
                if (rootBones == null || rootBones.Count == 0)
                {
                    ...
                    throw new VCIValidatorException(ValidationErrorType.RootBoneNotFound, springBone, errorText);
                }
                ...
                if (rootBones.Count > maxRootBoneCount)
                {
                    ...
                    throw new VCIValidatorException(ValidationErrorType.TooManyRootBone, springBone, errorText);
                }
                ...
                for (var i = 0; i < rootBones.Count; i++)
                {
                    ...
                    if (children.Length > maxChildBoneCount)
                    {
                        ...
                        throw new VCIValidatorException(ValidationErrorType.TooManyRootBoneChild, springBone, errorText);
                    }
                    ...
                    foreach (var childBone in children)
                    {
                        ...
                        if (childBone.GetComponent<VCISubItem>() != null)
                        {
                            ...
                            throw new VCIValidatorException(ValidationErrorType.RootBoneContainsSubItem, childBone, errorText);
                        }
                        ...
                        for (var j = 0; j < rootBones.Count; j++)
                        {
                            ...
                            if (rootBones[j] == childBone)
                            {
                                ...
                                throw new VCIValidatorException(ValidationErrorType.RootBoneNested, childBone, errorText);
                            }
                        }
                    }
                }
        ...
        private static void ValidatePlayerSpawnPoints(GameObject gameObject)
        {
            ...
            foreach (var playerSpawnPoint in playerSpawnPoints)
            {
                    ...
                    Math.Abs(spawnPointTransform.rotation.z) > 0.001f)
                {
                    ...
                    throw new VCIValidatorException(ValidationErrorType.SpawnPointNotHorizontal, playerSpawnPoint, errorText);
                }
                    ...
                    || spawnPointRestriction.LimitRectBackward > 0)
                {
                    ...
                    throw new VCIValidatorException(ValidationErrorType.SpawnPointOriginNotInRange, spawnPointRestriction, errorText);
                }
            }
        }
        ...
        private static void ValidateLocationBounds(GameObject gameObject)
        {
                ...
                Mathf.Abs(max.x) > 10000f || Mathf.Abs(max.y) > 10000f || Mathf.Abs(max.z) > 10000f)
            {
                ...
                throw new VCIValidatorException(ValidationErrorType.LocationBoundsValueExceeded, locationBounds, errorText);
            }
        }
        ...
        private static void ValidateEffekseer(GameObject gameObject)
        {
            foreach (var effekseerEmitter in gameObject.GetComponentsInChildren<EffekseerEmitter>())
            {
                ...
                var errorText = VCIConfig.GetText($"error{(int)ValidationErrorType.EffekseerMaterialIncluded}");
                throw new VCIValidatorException(ValidationErrorType.EffekseerMaterialIncluded, effekseerEmitter, errorText);
            }
        }

        private static void CheckInvalidComponent<T>(GameObject target) where T : Component
        {
            var c = target.GetComponentInChildren<T>(true);
            if (c == null) { return; }

            var gameObjectName = c.name;
            var errorText = VCIConfig.GetFormattedText($"error{(int)ValidationErrorType.InvalidComponent}", gameObjectName, typeof(T).Name);

            throw new VCIValidatorException(ValidationErrorType.InvalidComponent, c, errorText);
        }
    }
    ...
    public class VCIValidatorException : Exception
    {
        ...
        public VCIValidatorException(ValidationErrorType errorType, UnityEngine.Object obj) : base("")
        {
            ErrorType = errorType;
            Object = obj;
        }
        ...
        public VCIValidatorException(ValidationErrorType errorType, UnityEngine.Object obj, string message) : base(message)
        {
            ErrorType = errorType;
            Object = obj;
        }
